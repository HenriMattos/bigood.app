            ...
            {!isLoading && user ? (
              <div className="group relative">
                ...
                <div className="invisible absolute top-full right-0 z-50 mt-3 w-56 rounded-[24px] border border-[var(--landing-border)] bg-white p-2 opacity-0 shadow-[0_18px_45px_rgba(11,51,36,0.1)] transition-all group-focus-within:visible group-focus-within:opacity-100 group-hover:visible group-hover:opacity-100">
                  <div className="px-4 py-2 mb-1 border-b border-[var(--landing-border)]">
                    <p className="text-[11px] font-bold text-[var(--landing-muted)] uppercase tracking-wider">
                      Usuario
                    </p>
                    <p className="text-sm font-extrabold truncate text-[var(--landing-primary-dark)]">
                      {user.email}
                    </p>
                  </div>
                  <Link
                  ...
                  <button
                    onClick={logout}
                    className="block w-full text-left rounded-[18px] px-4 py-3 text-sm font-medium text-destructive hover:bg-destructive/10 focus-visible:ring-3 focus-visible:ring-ring/30 focus-visible:outline-none"
                  >
                    Sair
                  </button>
                </div>
              ...
              {!isLoading && user ? (
                <>
                  <div className="px-4 py-2 text-sm">
                    <p className="font-bold text-[var(--landing-muted)]">Logado como:</p>
                    <p className="font-extrabold truncate">{user.email}</p>
                  </div>
                  <Button variant="outline" className={darkOutlineButtonClass} asChild>
